...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(`https://fastapi.james-auble.workers.dev/api/search?q=${input}`)
                ...
                searchResults?.results ? (
                  <>
                    ...
                    <p className="p-2 text-xs text-zinc-500">
                      Found {searchResults.results.length} results found in {searchResults.duration.toFixed(0)}ms
                    </p>
                  </>
                ) : null
              }
